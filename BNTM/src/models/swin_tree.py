...
class ViT_NeT(nn.Module):
    def __init__(self, config):
        ...
        self.tree = DTree(num_classes=config.MODEL.NUM_CLASSES, tree_depth=config.MODEL.TREE.DEPTH, embed_dim=1024,
                          proto_size=config.MODEL.TREE.PROTO_SIZE)

    def forward(self, x):
        logits, patches = self.patch_embedding(x)
        out = self.tree(logits, patches, None, None)
        return out
    ...
    def explain(self, x, y, prefix):
        ...
        logits, patches = self.patch_embedding(x)
        self.tree.explain(logits, patches, x_np, y, None, prefix)
